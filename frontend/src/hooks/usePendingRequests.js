...
const usePendingRequests = () => {
  ...
  const postPendingRequest = async (user_id) => {
    ...
    setIsPending(true);
    try {
      const res = await fetch(`http://127.0.0.1:5000/api/v1/friendship/request/${user_id}`, {
        method: 'POST',
        headers: {
  ...
  const getPendingRequests = async () => {
    ...
    setIsPending(true);
    try {
      const res = await fetch(`http://127.0.0.1:5000/api/v1/explore/friendshipRequests`, {
        headers: {
          Authorization: `bearer ${user.token}`,
        },
      });
  ...
  const cancelPendingRequest = async (user_id) => {
    ...
    setIsPending(true);
    try {
      const res = await fetch(`http://127.0.0.1:5000/api/v1/friendship/cancelRequest/${user_id}`, {
        method: 'POST',
        headers: {
          Authorization: `bearer ${user.token}`,
        },
